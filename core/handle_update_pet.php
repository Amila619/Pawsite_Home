...
if (isset($_GET['img_url']) && isset($_GET['pet_id'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (
            ...
            $sql = "UPDATE pets SET name = ?, species = ?, breed = ?, age = ?, size = ?, color = ?, temperament = ?, health_status = ?, adoption_fee = ?, status = ?, description = ?, img_url = ? WHERE pet_id = ?";
            ...
            if ($stmt->execute()) {
                ...
                echo "<h3>Error Updating Pet: {$stmt->error}</h3><br>";
            }
